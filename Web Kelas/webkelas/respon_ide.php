...
if (!isset($id_siswa)) {
    echo "<script>alert('data siswa tidak valid'); window.location.href= 'view_ide.php'</script>";
    exit;
}

$sql = "SELECT id FROM ide WHERE siswa_id = {$id_siswa}";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE ide SET response_text='$response_text' WHERE siswa_id='$id_siswa' AND id='$id'";
    if ($koneksi->query($sql) === TRUE) {
        header('Location: view_ide.php');
    } else {
        header('Location: view_ide.php');
    }
    exit;
